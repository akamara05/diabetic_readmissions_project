...
After my initial cleaning steps I had cut the dataset to 98,052 and 48 features target variable feature to conduct EDA upon. 
...
![alt text](images/readmission.png "Readmission")

* __0:__ 53% of patients were not readmitted to a hospital after discharge. 
* __1:__ 47% of patients were readmitted to hospital after discharge. 


### Demographics

* __48 %__ of patients readmitted are between 60-80 years of age.  
* __53 %__ of readmitted patients are females. 
* __76%__ of readmitted patients are Caucasian.

### Diagnosis

![alt text](images/primary.png "Diagnosis")

There were a number of features to explore in the dataset but what was most striking to me was the primary diagnosis data. As mentioned earlier it’s key that a timely diagnosis is made by medical practitioners to help identify high risk patients and in turn reduce the occurrence of readmittance. The graph displayed provides an interesting story. When we take a look at it, it's surprising to note that diabetes itself does not lead the way in terms of primary diagnosis. In fact it is the least diagnosed. Instead patients are primarily diagnosed with a circulatory disease, such as  heart attacks, high-blood pressure,chest pains, and high cholesterol. 

### Addressing Outliers

__Lab Procedures Performed__

On average 43 lab tests were performed during a patient's initial admission with a max of 132 performed, for a lay person that seems like a lot of procedures to perform on one patient. So I decided to take a look at outliers, doing so indicated that anything past 96 lab procedures was outside the threshold. I removed outliers and looked to see the relationship with readmission rates, as you can see from the graph you can see a more uniform distribution and one that mirrors the readmission rates, indicating a stronger correlation to the target variable. 
...
The initial model was skewed to the left so I decided to delve deeper into this feature. On average 16 distinct generic types of medication were administered to the patient during their initial admission. A maximum of 81 distinct types of medication were administered which I also deemed to be a large amount for one patient and possibly not the norm, since that is a big jump from the average. A look at the outliers indicated that anything past 33 distinct types of medications falls out of the upper limit.
...
For each of the models I used these various methods to arrive at the best accuracy and sensitivity/ recall score as possible.:

1. Determining and using just the Most Important Features Using a Decision Tree
![alt text](images/important.png "Important Features")

2. Using principal Component Analysis
3. Leveraging GridSearchCV and fine tuning parameters to each of the model.
...
The results are consolidated in the table below. 
 ...
 |---|-----|:---------------:|:--------------:|:------:|:----------------------:|:------:|
 |1|Logistic Regression|0.6184|0.6180|0.6181|0.4336|Higher than baseline but not very high results. Just slightly higher than the baseline|
 |2|KNN|0.7202|0.5586|0.5632|0.4336|Expected that the testing score would be higher than testing score, since KNN models commonly tend to over-fit. Accuracy score lower than baseline score of 0.47|
 |3|Random Forest|0.9999|0.6218|0.6181|0.5151|Same accuracy scores as the logistic regression model. Overall scores aren't vastly higher than the baseline|
 |4|Logistic w/ Important Features|0.6090|0.6093|0.6083|-|With limited features, all scores compared to the initial models decrease|
 ...
 |8|KNN w/ PCA|-|-|-|0.5195|Although not the lowest accuracy score out of all the KNN model variations, the score did decrease in comparison to the  KNN model with limited features.|
 |9|Random Forest w/PCA|-|-|-|0.5166|Although not the lowest accuracy score out of all the random forest model variations, the score did decrease in comparison to the random forest model with limited features.|
 |10|Logistic Regression w/Grid Search|0.6089|0.6093|0.6087|0.3866|Produced the lowest sensitivity/recall score|
 |11|KNN w/ Grid Search|0.7708|-|-|0.5195| Lower sensitivity/recall score than the PCA variation of the model|
...
* The scope of the data is from 1999-2008, so I'd be interested in fining more recent data, if available and taking similar steps to see if there are any improvement in the my model success rates. 
* The inclusion of more features would most likely be required since I had stripped my features to just slightly more than 30 features which I believe was the cause of the consistent underperformance of my three models for identifying patient readmission. Maybe additional data such as family history  may need to be included to a dataset such as this. In a lot of medical cases, family history seems to play a major role in diagnosing patients especially with hereditary diseases, such as Type 2 diabetes in this case. 
* As predictable as this may be, but given the current COVID pandemic, I’d be interested in exploring how these past 2 years have affected readmission rates.


## References 

[1] Center for Disease Control and prevention (2021),"Diabetes Basics.", https://www.cdc.gov/diabetes/basics/getting-tested.html
